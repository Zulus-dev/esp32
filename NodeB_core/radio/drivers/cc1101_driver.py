...
#
# RAW capture rules:
# - Hard wall-clock 1600 ms — fills up to 8 KB @ 25 µs/sample; under Node B WDT 12 s.
# - Bit timing via ticks_us (not busy for-range).
# - No heap in hot path. Feed WDT around the call, not inside the sample loop.
#
...
_SAMPLE_US = 25
# Ceiling for one raw_capture call (ms). 1600 ms × 40k samples/s ≈ 64k bits ≈ 8 KB.
# Must stay well under Node B WDT (12000 ms).
_CAPTURE_HARD_MS = 1600
