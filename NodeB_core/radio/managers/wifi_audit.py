...
class Manager:
    def __init__(self, link): self.link=link; self.wlan=None; self.task=None; self._tlv=bytearray(40)
    ...
    async def _scan_once(self):
        ...
        for ap in aps:
            ...
            n=len(ssid); p=self._tlv; p[0]=MOD_WIFI; p[1]=0; p[2]=ch&255; p[3]=rssi&255; p[4]=auth&255; p[5]=n; p[6:6+n]=ssid
            self.link.send_tlv(TLV_SCAN_RESULT, memoryview(p)[:6+n]); c+=1; await asyncio.sleep_ms(8)
