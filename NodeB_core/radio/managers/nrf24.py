...
class Manager:
    def __init__(self, link):
        self.link=link; self.drv=None; self.task=None; self.buf=None; self.mode=NRF_MODE_RX; self.channel=2
    ...
    def _evt(self, ev, pipe, ch, rssi, data=None, n=0):
        if data is None: n=0
        if n>32: n=32
        p=bytearray(5+n); p[0]=ev; p[1]=pipe; p[2]=ch; p[3]=rssi & 255; p[4]=n
        if n: p[5:]=memoryview(data)[:n]
        self.link.send_tlv(TLV_NRF_EVENT,p)
