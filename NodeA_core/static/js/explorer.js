...
const FS_LIST_MAGIC = 0xF51E;
const FS_TYPE_FILE = 0, FS_TYPE_DIR = 1, FS_TYPE_BACK = 2;
...
function decodeList(buf) {
  ...
  if (u.length < 4) throw new Error('short_list');
  const magic = (u[0] << 8) | u[1];
  if (magic !== FS_LIST_MAGIC) throw new Error('bad_magic');
  const pl = u[2];
  let pos = 3;
  const path = new TextDecoder().decode(u.subarray(pos, pos + pl));
  pos += pl;
  const count = u[pos++];
  const items = [];
  for (let i = 0; i < count && pos < u.length; i++) {
    const typ = u[pos++];
    const size = ((u[pos] << 24) | (u[pos + 1] << 16) | (u[pos + 2] << 8) | u[pos + 3]) >>> 0;
    pos += 4;
    const nl = u[pos++];
    const name = new TextDecoder().decode(u.subarray(pos, pos + nl));
    pos += nl;
    let type = 'file', dir = false;
    if (typ === FS_TYPE_DIR) { type = 'dir'; dir = true; }
    else if (typ === FS_TYPE_BACK) { type = 'back'; dir = true; }
    const full = (type === 'back') ? (path === '/' ? '/' : path.replace(/\/[^/]+$/, '') || '/') : joinPath(path, name);
    items.push({ name: type === 'back' ? '..' : name, path: type === 'back' ? (path === '/' ? '/' : path.replace(/\/[^/]*$/, '') || '/') : joinPath(path, name), type, dir, size });
  }
  return { path, items };
}
...
async function api(url, options) {
  ...
  if (ct.indexOf('application/octet-stream') >= 0 || ct.indexOf('octet-stream') >= 0) {
...
async function refreshPane(n) {
  const buf = await api(`/api/list?path=${enc(paths[n])}`);
  const data = decodeList(buf);
