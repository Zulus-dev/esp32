# web/server.py — single-flight HTTP + mempool TX slab
#
# === RAM RULES (обязательно при любых правках) ===
# 1. Один запрос в полёте (_busy). backlog=1. Не поднимать concurrency.
# 2. TX только mempool "web_tx" / fallback static — не bytearray() на каждый chunk.
# 3. После каждого клиента: writer close + gc.collect()×2.
# 4. При free < CRITICAL — 503, не обслуживать body.
# 5. Новые handlers: стримить; запрещены большие json.dumps и list-буферы.
#
...
class AsyncServer:
    def __init__(self, port=80, core=None):
        ...
        self._busy = False
        self._tx = None  # slab from mempool
    ...
    async def start(self):
        ...
        if self._tx is None:
            self._tx = mempool.register("web_tx", 512, sticky=True)
    ...
    async def stop(self):
        ...
        try:
            from lib import mempool
            mempool.release("web_tx")
            mempool.release("web_hdr")
            mempool.release("web_json")
        except Exception:
            pass
        self._tx = None
        gc.collect()
    ...
    async def handle_client(self, reader, writer):
        # Queue parallel browser requests — only one body in flight
        waited = 0
        while self._busy:
            ...
            if waited > 3000:
                try:
                    ...
                    pass
                return
        # Soft RAM gate: avoid deep heap dig that ends in reset
        try:
            from lib import mempool
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    pass
            self._busy = False
            # Double collect: stream objects often free on second pass
            gc.collect()
            gc.collect()
    ...
    async def _serve_static(self, url, writer):
        ...
        try:
            ...
            # Cap pathological huge files in one response
            if size > 64 * 1024:
